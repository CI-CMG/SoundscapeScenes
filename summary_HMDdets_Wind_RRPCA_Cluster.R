...
clust$dateTime <- as.POSIXct(   paste(clust$year, clust$month, clust$day,
        clust$hour, clust$minute, clust$second),   format = "%Y %m %d %H %M %S")
min(clust$dateTime) 
max(clust$dateTime)
#truncate cluster to only time in the current deployment... no overlap???
idx = which (clust$dateTime >= min(HMDdet$dateTime)  & clust$dateTime <= max(HMDdet$dateTime) )  
clustT = clust[idx,]
merged_df <- merge(HMDdet, clustT, by = "dateTime", all.x = TRUE)
...
(unique( HMDdet$RRPCAw))

### START HERE!!!
# PLOT: what acoustic scenes do the RRPCA results fall into
ggplot(HMDdet, aes(fill = RRPCAw, x = SSD)) +
  geom_bar(position="fill") +
  theme_minimal()
library(ggmosaic)
#mosaicplot(SSD ~ RRPCAw, data = HMDdet) 
...
if (length(inFilesW) > 0 ) {
  ...
  HMDdet$Category3 = paste(HMDdet$windCat, HMDdet$Category2, sep = "_" )
  idNa = which ( is.na(HMDdet$wind_speed) )
